...
function Adoption() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if(!formData.address.trim()) { //if no string is cut means no text entered
        validationErrors.address = "!"
        }
        if(!formData.contact.trim()) { //if no string is cut means no text entered
        validationErrors.contact = "!"
        }
        if(!formData.household.trim()) { //if no string is cut means no text entered
            validationErrors.household = '!'
        }
        if(!formData.employment.trim()) { //if no string is cut means no text entered
            validationErrors.employment = '!'
        }
        if(!formData.pet_exp.trim()) { //if no string is cut means no text entered
            validationErrors.pet_exp = '!'
        }
        if(formData.household.length > 500) { //above 500 characters
        validationErrors.household = '!'
        }
        if(formData.employment.length > 500) { //above 500 characters
            validationErrors.employment = '!'
        }
        if(formData.pet_exp.length > 500) { //above 500 characters
            validationErrors.pet_exp = '!'
        }    
        setErrors(validationErrors);
        console.log(petData.state.pet)
            ...
            try {
                ...
                .then((res) => {
                    if(!res.data.message){
                        ...
                        alert('An email will be sent to the address you provided within 48 hours regarding the approval status. Please check your inbox (and spam/junk folder) for our message. If you have any questions in the meantime, feel free to contact us at furrMe@gmail.com.');
                    }
                })
        ...
        <div>
            ...
            <div className="adoptionVerificationContainer">
                ...
                <div className="adoptionVerification">
                    ...
                    <div className="addressWarning">{errors.address && <span> {errors.address} </span>}</div>
                    <div className="contactWarning">{errors.contact && <span> {errors.contact} </span>}</div>
                    <div className="householdWarning">{errors.household && <span> {errors.household} </span>}</div>
                    <div className="employmentWarning">{errors.employment && <span> {errors.employment} </span>}</div>
                    <div className="petExpWarning">{errors.pet_exp && <span> {errors.pet_exp} </span>}</div>
