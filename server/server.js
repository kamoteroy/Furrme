...
app.get('/petDetails/:id', async (req, res) => {
    db.query("select * from pets where pet_id = ?", req.params['id'], (err, pets) => {
        if(err){
            console.log(err);
        }
        else{
...
app.get('/petImage/:id', async (req, res) => {
    db.query("select img1,img2,img3,img4,img5 from pet_img where pet_id = ?", req.params['id'], (err, pets) => {
        if(err){
            console.log(err);
        }
        else{
...
app.post('/admin/preview', (req, res) => {
    db.query("select * from accounts where email = ?", [req.body.email], (err, accInfo) => {
        if(err){
            console.log(err);
        }
        else{
...
app.post('/adoptreq', async (req, res) => {
    const addpost = "INSERT into adoptreq(pet_id, pet_name, category, email, image, address, contact, household, employment, pet_exp, dates, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    db.query(addpost, [req.body.id, req.body.name, req.body.category, req.body.email, req.body.image ,req.body.address, req.body.contact, 
        req.body.household, req.body.employment, req.body.pet_exp, req.body.date, req.body.status], (err, result) => {
...
app.get('/admin', (req, res) => {
    db.query("select * from pets inner join pet_img on pets.pet_id = pet_img.pet_id", (err, petList) => {
        if(err){
            console.log(err);
        }
        else{
...
app.post('/admin/evaluation/pet', (req, res) => {
    db.query("select * from pets where pet_id = ?", [req.body.id], (err, petInfo) => {
        if(err){
            console.log(err);
        }
...
app.post('/admin/setStatus', (req, res) => {
    db.query("UPDATE adoptreq SET status = ? WHERE pet_id = ? and email = ?", [req.body.status, req.body.id, req.body.email], (err, accInfo) => {
        if(err){
            console.log(err);
        }
...
app.post('/petpreview/:name', (req, res) => {
    db.query("select * from pet_img where pet_id = ?", [req.body.petID], (err, pets) => {
        if(err){
            console.log(err);
        }
        else{
